...
Opin은 Pinterest 핀의 원본 고해상도 이미지를 바로 열 수 있게 해 주는 브라우저 확장 프로그램입니다.
Pinterest의 **저장** 버튼 옆에 버튼을 추가하며, 클릭하면 원본 이미지가 새 탭으로 열립니다.
...
확장 프로그램의 소스 코드는 공개 배포하지 않습니다.
Opin은 아래에 안내된 공식 브라우저 확장 프로그램 스토어를 통해서만 설치해 주세요.
...
Opin은 어떠한 개인정보도 수집·저장하지 않으며, 외부 서버와 통신하지 않습니다.
자세한 내용은 [개인정보처리방침](PRIVACY.ko.md)을 참고하세요.
...
[MIT 라이선스](../LICENSE)는 이 저장소에 공개된 문서 및 기타 파일에만 적용됩니다.
Opin 브라우저 확장 프로그램과 그 소스 코드에는 적용되지 않습니다.
